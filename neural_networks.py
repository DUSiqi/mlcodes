...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        # print("weights W in linear init")
        # print(self.params['W'])
        # print("biases b in linear init")
        # print(self.params['b'])
        ...
        # print("gradient W in linear init")
        # print(self.gradient['W'])
        # print("gradient b in linear init")
    ...
    def backward(self, X, grad):
        ...
        # sum函数，axis = 0 是按列求和，axis = 1 是按行求和
        # self.gradient['b'] = np.sum(grad,axis=0).reshape((1,-1))
        self.gradient['b'] = np.sum(grad, axis=1,keepdims=True)
        backward_output = np.dot(grad,self.params['W'].T)

        # print("在backward里输出gradient W")
        # print(self.gradient['W'].shape)
        # print("在backward里输出gradient b")
        # print(self.gradient['b'].shape)
        # print("在backward里输出backward_output")
        # print(backward_output.shape)
        #############################################V2##################
        # self.gradient['W'] = (np.multiply(X, grad)).T
        # self.gradient['b'] = np.sum(grad, axis=0).reshape((1, -1))
...
class relu:
    ...
    def forward(self, X):
        ...
        # print(X.shape, X)
        # print("在relu中打印mask：")
        # print(self.mask.shape,self.mask)
        # print("在relu中打印forward_output结果：")
        # print(forward_output.shape,forward_output)
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        gradient_relu = self.mask * 1
        backward_output = gradient_relu * grad

        # print("在relu反向传播中打印relu的梯度数组：")
        # print(gradient_relu.shape,gradient_relu)
        # print("在relu反向传播中打印backward_output：")
        # print(backward_output.shape,backward_output)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = grad * (1 - np.tanh(X)*np.tanh(X))
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    module.params[key] = _ - _learning_rate * g
    return model
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        if (t % _step == 0) and (t != 0):  #每10个epoch，learning_rate *= 0.1
            _learning_rate = _learning_rate * 0.1

        idx_order = np.random.permutation(N_train)   # N_train = N
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):    # 每轮都训练minibatch_size的数量，知道把一整个epoch训练完
            ...
            d1 = model['drop1'].forward(h1, is_train = True)  # if false, no dropout
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            a1 = model['L1'].forward(x)
            h1 = model['nonlinear1'].forward(a1)
            d1 = model['drop1'].forward(h1, is_train=False)  # if false, no dropout
            a2 = model['L2'].forward(d1)
